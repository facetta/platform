...
Platform.prototype.setModuleOptions = function(options) {
  this._moduleOptions = _.extend(this._moduleOptions, options);

  // ensure a db key exists
  if( !this._moduleOptions.db ) {
    var mongoose = require('mongoose');
    mongoose.connect( options.dbServer, { server: { socketOptions: { keepAlive: 1 } } });
    mongoose.connection.on( 'error', console.error.bind( console, 'connection error:' ) );  
    this._moduleOptions.db = mongoose;
  }
...
Platform.prototype.init = function(express) {
  ...
  var responseHandler = new ResponseHandler(this._moduleOptions);
  ...
  express.use(this._apiInstances[first].facetInit());
};
